...
resource "aws_instance" "docker-machine-managers" {
  ...
  tags = {
    Name = "matts-Docker-Swarm-Manager-${count.index + 1}"
    server = "matts-docker-manager-${count.index + 2}"
    project = "205"
  }
...
resource "aws_instance" "docker-machine-workers" {
  ...
  tags = {
    Name = "matts-Docker-Swarm-Worker-${count.index + 1}"
    server = "matts-docker-worker-${count.index + 1}"
    project = "205"
  }
  depends_on = [aws_instance.docker-machine-leader-manager]
}
...
resource "aws_security_group" "tf-docker-sec-gr" {
  name = "matts-docker-swarm-sec-gr"
  tags = {
    Name = "matts-swarm-sec-gr"
  }
  dynamic "ingress" {
